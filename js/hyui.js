$(function () {
  ...
  (_searchebook = $('.search_ebook')), _sidebarCtrl.append('<span></span><span></span><span></span>');
  ...
  function mobileMenu() {
    ...
    if (ww < wwMedium) {
      ...
      hideSidebar();
      _body.removeClass('noscroll');
      _menu.appendTo('.header .container .header_rightdata');
      _nav.appendTo('.header .container .header_rightdata');
  ...
  $('a.goCenter').keydown(function (e) {
    if (e.which == 13) {
      ...
      $('html, body')
        .stop(true, true)
        .animate({ scrollTop: $('.main').find('.accesskey').offset().top - 150 }, 800, 'easeOutExpo');
    }
  });
  /*-----------------------------------*/
  //////// 語言模組 無障礙遊走設定  ////////
  ...
  $('table').each(function (index, el) {
    //判斷沒有table_list
    if ($(this).parents('.table_list').length == 0 && $(this).parents('.fix_th_table').length == 0 && $(this).parent('form').length == 0 && $('table').parents('.advanced_group').length == 0 && $('table').parents('.respondent_information').length == 0) {
      $(this).scroltable();
    }
  });
